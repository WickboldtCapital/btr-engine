...
long_term_intro = (
    ...
    f"**{appreciation_rate*100:.1f}% annual asset appreciation**, continuous principal paydown funded directly by your tenants, "
    f"and an **annual Tax Shelter of ${annual_tax_savings:,.0f}** generated by the 27.5-year depreciation schedule shielding your ordinary income."
)
...
# --- NEW CHART: THE 4 PILLARS OF WEALTH (YEAR 1) ---
...
tot_ret_viz_data = pd.DataFrame({
    "Wealth Pillar": ["1. Net Cash Flow", "2. Principal Paydown", "3. Asset Appreciation", "4. Tax Savings (Depreciation)"],
    "Amount ($)": [yr1_cf, yr1_prin, yr1_appr, yr1_tax],
    "Category": ["Year 1 Total Return"] * 4
})

fig_tot_ret = px.bar(tot_ret_viz_data, x="Category", y="Amount ($)", color="Wealth Pillar", barmode="stack",
                     text="Amount ($)", title=f"Year 1 Total Return: ${total_yr1_return:,.0f} (The 4 Pillars of Wealth)",
                     color_discrete_sequence=["#2ca02c", "#1f77b4", "#ff7f0e", "#9467bd"])
fig_tot_ret.update_traces(texttemplate='$%{text:,.0f}', textposition='inside', insidetextanchor='middle')
fig_tot_ret.update_layout(xaxis_title="", yaxis_title="Generated Wealth ($)")
...
fig_wealth.update_layout(title="Generational Wealth Creation (30-Year Equity Accumulation)", xaxis_title="Years After Stabilization", yaxis_title="Portfolio Value ($)", hovermode="x unified")
...
with st.expander("📊 View 5-Year and 10-Year Projected Returns", expanded=False):
    projection_data = {
        "Metric": [
            ...
            "Cumulative Tax Savings (Depreciation Shield)",
            "Cumulative Total Return (The 4 Pillars)",
        ...
        "Year 1": [
            ...
            f"${annual_tax_savings:,.0f}",
            f"${cumulative_cf[1] + principal_paid[1] + (asset_vals[1] - total_arv) + annual_tax_savings:,.0f}",
            "-"
        ],
        "Year 5": [
            f"${asset_vals[5]:,.0f}",
            ...
            f"${annual_tax_savings * 5:,.0f}",
            f"${cumulative_cf[5] + principal_paid[5] + (asset_vals[5] - total_arv) + (annual_tax_savings * 5):,.0f}",
            irr_5_str
        ],
        "Year 10": [
            f"${asset_vals[10]:,.0f}",
            ...
            f"${annual_tax_savings * 10:,.0f}",
            f"${cumulative_cf[10] + principal_paid[10] + (asset_vals[10] - total_arv) + (annual_tax_savings * 10):,.0f}",
            irr_10_str
        ]
    }
